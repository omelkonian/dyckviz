...
class Web(object):
    ...
    def render_orbit(word, output):
        ...
        merger.write('{}/equiv_class.pdf'.format(output))
...
def render_all_orbits(k, n, output):
    count_orbits(k, n)
    for i, orbit in enumerate(all_orbits(k, n), start=1):
        ...
        if k == 3:
            ...
            for filename in glob.glob('webs/*.pdf'):
                merger.addpages(PdfReader(filename).pages)
            merger.write('{}/equiv_class_{}.pdf'.format(output, i))
            for filename in glob.glob('webs/*.pdf'):
                os.remove(filename)
            shutil.rmtree('webs')
...
def main():
    ...
    if args.dim:
        ...
        render_all_orbits(k, n, output=args.o)
    elif args.word:
        render_promotion(args.word, 3)
        Web.render_orbit(args.word, output=args.o)
